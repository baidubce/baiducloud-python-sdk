"""
DocAnalysisOfficeSecIdx information
"""

from baiducloud_python_sdk_core.abstract_model import AbstractModel


class DocAnalysisOfficeSecIdx(AbstractModel):
    """
    DocAnalysisOfficeSecIdx
    """

    def __init__(self, idx=None, para_idx=None, row_idx=None, col_idx=None):
        """
        Initialize DocAnalysisOfficeSecIdx instance.

        :param idx: 每个属性下包含的文本行id序号
        :type idx: List[float] (optional)

        :param para_idx: 当attribute=section时返回，表示section中包含的9个模块的返回顺序号id
        :type para_idx: List[float] (optional)

        :param row_idx: 当attribute=section时返回，表示所属网格的行的id
        :type row_idx: List[float] (optional)

        :param col_idx: 当attribute=section时返回，表示所属网格的列的id
