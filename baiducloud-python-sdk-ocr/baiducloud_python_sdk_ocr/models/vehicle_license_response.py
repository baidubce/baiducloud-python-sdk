"""
Request entity for VehicleLicenseResponse information.
"""

from baiducloud_python_sdk_core.bce_response import BceResponse
from baiducloud_python_sdk_ocr.models.vehicle_license_quality_propobility import VehicleLicenseQualityPropobility


class VehicleLicenseResponse(BceResponse):
    """
    ...
    def __init__(
        ...
        """
        Initialize VehicleLicenseResponse response.
        ...
        :param direction: 图像方向，当detect_direction=true时返回该字段
        :type direction: int (optional)

        :param words_result_num: 识别结果数，表示words_result的元素个数
        :type words_result_num: int (optional)

        :param words_result: 识别结果，key为字段名（如：号牌号码、车辆类型等），value为识别内容
        :type words_result: object (optional)

        :param warn_infos: 质量告警信息，当请求参数vehicle_license_side=front且quality_warn=true时输出
        :type warn_infos: List[str] (optional)

        :param quality_propobility: quality_propobility field
        :type quality_propobility: VehicleLicenseQualityPropobility (optional)

        :param risk_type: 当输入参数risk_warn=true时输出
        :type risk_type: str (optional)

        :param edit_tool: 当输入参数risk_warn=true时返回，如果检测行驶证被编辑过，该字段指定编辑软件名称
    ...
    def from_dict(self, m):
        ...
        if m.get('quality_propobility') is not None:
            self.quality_propobility = VehicleLicenseQualityPropobility().from_dict(m.get('quality_propobility'))
