"""
Request entity for GetSandboxSnapshotResponse information.
"""
...
class GetSandboxSnapshotResponse(BceResponse):
    """
    GetSandboxSnapshotResponse
    """

    def __init__(self, sandbox_id=None, snapshot=None):
        """
        Initialize GetSandboxSnapshotResponse response.

        :param sandbox_id: 沙箱实例 ID。
        :type sandbox_id: str (optional)

        :param snapshot: snapshot field
        :type snapshot: SnapshotInfo (optional)
        """
        super().__init__()
    ...
    def from_dict(self, m):
        ...
        if m.get('snapshot') is not None:
            self.snapshot = SnapshotInfo().from_dict(m.get('snapshot'))
        return self
