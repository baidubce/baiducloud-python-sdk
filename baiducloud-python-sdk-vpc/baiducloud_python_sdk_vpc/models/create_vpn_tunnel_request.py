...
class CreateVpnTunnelRequest(AbstractModel):
    ...
    def __init__(self, vpn_id, secret_key, local_subnets, cgw_id, remote_subnets, vpn_conn_name, ike_config, ipsec_config, client_token=None, description=None):
        """
        初始化创建VPN隧道请求
        
        Args:
            vpn_id (str): VPN网关ID
            secret_key (str): 预共享密钥
            local_subnets (list): 本地子网列表
            cgw_id (str): 客户网关ID
            remote_subnets (list): 对端子网列表
            vpn_conn_name (str): VPN连接名称
            ike_config (IkeConfig): IKE配置
            ipsec_config (IpsecConfig): IPSEC配置
            client_token (str, optional): 幂等性token
    ...
    def from_dict(self, m):
        ...
        Returns:
            CreateVpnTunnelRequest: 初始化后的对象
        """
        m = m or dict()
