"""
Request entity for CreateBlbRequest information.
"""
...
class CreateBlbRequest(AbstractModel):
    """
    Request entity for CreateBlbRequest operation.
    ...
    def __init__(
        ...
        """
        Initialize CreateBlbRequest request entity.
        ...
        :param vpc_id: LoadBalancer实例所属VPC的ID
        :type vpc_id: str (required)

        :param subnet_id: LoadBalancer实例所属子网的ID
        :type subnet_id: str (required)

        :param address: 指定负载均衡实例的私网IP地址，该地址必须包含在子网网段下。
        :type address: str (optional)

        :param type: 指定负载均衡实例的类型，不传默认为普通型，传\"ipv6\"代表创建ipv6类型的BLB。
