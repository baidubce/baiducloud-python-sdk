"""
Request entity for QueryEipListResponse information.
"""
...
class QueryEipListResponse(BceResponse):
    """
    QueryEipListResponse
    """

    def __init__(self, eip_list=None, marker=None, is_truncated=None, next_marker=None, max_keys=None):
        """
        Initialize QueryEipListResponse response.

        :param eip_list: 包含查询结果的列表
        :type eip_list: List[EipModel] (optional)
    ...
    def from_dict(self, m):
        ...
        if m.get('eipList') is not None:
            self.eip_list = [EipModel().from_dict(i) for i in m.get('eipList')]
