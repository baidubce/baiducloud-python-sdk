"""
Request entity for CreateDedicatedChannelRequest information.
"""
...
class CreateDedicatedChannelRequest(AbstractModel):
    """
    Request entity for CreateDedicatedChannelRequest operation.
    ...
    def __init__(
        ...
        """
        Initialize CreateDedicatedChannelRequest request entity.
        ...
        :param networks: 路由参数，仅当“routeType”为“static-route”时需要
        :type networks: List[str] (optional)

        :param customer_address: IDC互联IP
        :type customer_address: str (required)

        :param route_type: 路由协议，当前支持”static-route“（静态）和”bgp“（动态）
        :type route_type: str (required)

        :param vlan_id: VLAN ID，取值范围：0, 2-4009
        :type vlan_id: int (required)

        :param bgp_asn: bgp_asn parameter
        :type bgp_asn: str (optional)

        :param bgp_key: BGP 密钥，仅当“routeType”为“bgp”时需要
        :type bgp_key: str (optional)
    ...
    def from_dict(self, m):
        ...
        :rtype: CreateDedicatedChannelRequest
        ...
        if m.get('bgpKey') is not None:
            self.bgp_key = m.get('bgpKey')
