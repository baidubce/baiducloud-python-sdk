"""
PlannedEventResponse information
"""
...
class PlannedEventResponse(BceResponse):
    """
    ...
    def __init__(
        ...
        """
        Initialize PlannedEventResponse instance.

        :param server_event_id: 事件id（查询预期事件列表、查询预期时间记录列表接口返回）
        ...
        :param instance_id: 故障实例ID（查询预期事件列表、查询预期时间记录列表接口返回）
        :type instance_id: str (optional)

        :param product_category: 故障实例产品类型（查询预期事件列表、查询预期时间记录列表接口返回）
        :type product_category: str (optional)

        :param instance_spec: 故障实例规格（查询预期事件列表、查询预期时间记录列表接口返回）
        :type instance_spec: str (optional)

        :param instance_name: 故障实例名（查询预期事件列表、查询预期时间记录列表接口返回）
        :type instance_name: str (optional)

        :param private_ip: 故障实例的内网IP。（查询预期事件列表、查询预期时间记录列表接口返回）
        ...
        :param maintenance_options: 该事件支持的运维操作，可能支持的类型：Repair、Reboot（查询预期事件列表、查询预期时间记录列表接口返回）
        ...
        :param authorized_maintenance_operation: 该事件已授权的维修方式，与授权时使用的方式一致（查询预期事件列表、查询预期时间记录列表接口返回）
        :type authorized_maintenance_operation: str (optional)

        :param associated_planned_maintenance_server_event_ids: 多事件情况下该实例关联计划内运维事件ID列表（查询预期事件列表、查询预期时间记录列表接口返回）
        :type associated_planned_maintenance_server_event_ids: List[str] (optional)

        :param associated_unplanned_maintenance_server_event_ids: 多事件情况下该实例关联非预期运维事件ID列表（查询预期事件列表、查询预期时间记录列表接口返回）
        ...
        :param server_event_logs: 操作日志，包括用户授权、运维、验收等操作记录。（查询预期事件列表、查询预期时间记录列表接口返回）
        :type server_event_logs: List[OperationRecordResponse] (optional)

        :param has_fast_repair_stock: 是否有快速修复库存（查询预期事件列表、查询预期时间记录列表接口返回）
        :type has_fast_repair_stock: bool (optional)

        :param risks: 事件风险项（查询预期事件列表、查询预期时间记录列表接口返回）
        :type risks: List[IssueResponse] (optional)
        """
        super().__init__()
    ...
    def to_dict(self):
        ...
        if self.risks is not None:
            result['risks'] = [i.to_dict() for i in self.risks]
        return result
    ...
    def from_dict(self, m):
        ...
        if m.get('risks') is not None:
            self.risks = [IssueResponse().from_dict(i) for i in m.get('risks')]
        return self
