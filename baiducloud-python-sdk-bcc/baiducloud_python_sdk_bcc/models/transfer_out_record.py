...
class TransferOutRecord(AbstractModel):
    """
    TransferOutRecord
    """
    ...
    def __init__(
        ...
        """
        Initialize TransferOutRecord instance.

        :param transfer_record_id: 券转移记录id
        :type transfer_record_id: str (optional)

        :param recipient_user_id: 接收人账号(脱敏处理)
        :type recipient_user_id: str (optional)
    ...
    def to_dict(self):
        ...
        if self.recipient_user_id is not None:
            result['recipientUserId'] = self.recipient_user_id
    ...
    def from_dict(self, m):
        ...
        if m.get('recipientUserId') is not None:
            self.recipient_user_id = m.get('recipientUserId')
