...
class AlarmEventDetail(AbstractModel):
    """
    AlarmEventDetail
    """
    ...
    def __init__(
        ...
        """
        Initialize AlarmEventDetail instance.
        ...
        :param end_time: 报警关闭时间，UTC时间，未关闭时为空字符串
        :type end_time: str (optional)

        :param duration: 报警持续时间，单位：分钟
        :type duration: int (optional)

        :param init_state: 初始报警状态，可选项：`ALERT` - 报警中，`NODATA` - 无数据报警
        :type init_state: str (optional)

        :param state: state attribute
        :type state: str (optional)

        :param close_reason: 报警关闭原因，可选项：`POLICY_MODIFIED` - 报警策略更新
        :type close_reason: str (optional)

        :param current_value: 报警异常值描述
