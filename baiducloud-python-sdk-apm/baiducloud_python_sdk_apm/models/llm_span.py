...
from baiducloud_python_sdk_apm.models.span_content_ref import SpanContentRef

from baiducloud_python_sdk_apm.models.span_content_ref import SpanContentRef

from baiducloud_python_sdk_apm.models.span_message import SpanMessage
...
class LLMSpan(AbstractModel):
    ...
    def __init__(
        ...
        :type input: SpanContentRef (optional)

        :param output: output attribute
        :type output: SpanContentRef (optional)

        :param input_messages: 输入消息列表
        :type input_messages: List[SpanMessage] (optional)

        :param output_messages: 输出消息列表
        :type output_messages: List[SpanMessage] (optional)

        :param attributes: attributes attribute
        :type attributes: Dict[str, str] (optional)

        :param resource: OTEL Resource对象
        :type resource: Dict[str, str] (optional)

        :param events: 事件列表
        :type events: List[SpanEvent] (optional)

        :param height: 当前span在瀑布图中的高度
        :type height: int (optional)

        :param sub_spans: 子span列表
        :type sub_spans: List[LLMSpan] (optional)
        """
        super().__init__()
    ...
    def to_dict(self):
        ...
        if self.input_messages is not None:
            result['inputMessages'] = [i.to_dict() for i in self.input_messages]
        if self.output_messages is not None:
            result['outputMessages'] = [i.to_dict() for i in self.output_messages]
    ...
    def from_dict(self, m):
        ...
        if m.get('input') is not None:
            self.input = SpanContentRef().from_dict(m.get('input'))
        if m.get('output') is not None:
            self.output = SpanContentRef().from_dict(m.get('output'))
        if m.get('inputMessages') is not None:
            self.input_messages = [SpanMessage().from_dict(i) for i in m.get('inputMessages')]
        if m.get('outputMessages') is not None:
            self.output_messages = [SpanMessage().from_dict(i) for i in m.get('outputMessages')]
        ...
        if m.get('subSpans') is not None:
            self.sub_spans = [LLMSpan().from_dict(i) for i in m.get('subSpans')]
        return self
