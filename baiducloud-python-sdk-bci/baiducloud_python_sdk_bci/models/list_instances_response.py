...
from baiducloud_python_sdk_bci.models.instance_model import InstanceModel
...
class ListInstancesResponse(BceResponse):
    ...
    def __init__(self, marker=None, is_truncated=None, next_marker=None, max_keys=None, result=None):
        ...
        :param is_truncated: true表示后面还有数据，false表示已是最后一页
        :type is_truncated: bool (optional)

        :param next_marker: 获取下一页所需传递的marker值。当isTruncated为false时不出现
        :type next_marker: str (optional)

        :param max_keys: 每页包含的最大数量
        :type max_keys: int (optional)

        :param result: BCI实例信息列表
